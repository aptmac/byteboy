...
Currently, Byteboy has several limitations that confines it's use cases. For starters, Byteboy accepts a Java file with the extension {\tt .java} or {\tt .class} as input, however, many larger applications are not simply launched through the command-line using the {\tt java} command. Many popular Java applications may be packaged in {\tt .jar} format as an executable, or rely on larger build systems such as Maven and Gradle which makes the situation trickier. 
